...
#   ------------------
#
#   some constraints on B_{1Mpc}
#                   n=2                     n=-2.9
#   Helical         <-1.378 (log-scale)
#   Non-helical     <-1.493 (log-scale)
#
# -------------------------------------------------------
import numpy as np
...
#### QUICK PLOT TO TEST THINGS
if do_initial_plot:
    plt.loglog(ells, 0.1*dls_poiss, label='poisson')
    plt.loglog(ells, 0.0094*dls_galdust, label='dust')
    plt.loglog(ells, dls_tens, label='tensor')
    plt.loglog(ells, dls_lens, label='lensing')
    markers = ['o', 'v', 's']
...
samples = sampler.get_chain(discard=1000, thin=10)
# print('samples shape:', samples.shape)
# print('printinsamples:')
# for i in range(flat_samples.shape[0]):
#     print(flat_samples[i,:])
